...
两个传感器的坐标先进行居中滑动平均。每个候选窗口均作用于完整流程：时间偏差估计、空间差异计算、分块系统偏差检验、随机波动计算和 10 Hz 融合均使用平滑后的序列。正式方案采用 5 点预平滑，最终融合轨迹另固定使用 11 点居中滑动平均；该输出后处理不参与固定系统偏差检验。

估计时间偏差时，以传感器 1 的原始采样时刻为比较点，对传感器 2 做线性插值，并最小化去除平均空间偏移后的二维残差：
...
因此负值表示方式 2 的时间轴需要向后平移。5 点预平滑下，最终估计约为 `-367.906 s`。
...
| -------------: | -------------: | -------: | ------------: | ------------: | --------------: | --------------: |
|              1 |       -367.944 |   46.361 |        -0.123 |        -0.171 | [-0.386, 0.140] | [-0.445, 0.103] |
|              3 |       -367.996 |   17.238 |        -0.111 |        -0.182 | [-0.367, 0.144] | [-0.439, 0.074] |
|              5 |       -367.906 |   10.527 |        -0.128 |        -0.164 | [-0.369, 0.113] | [-0.409, 0.082] |
|              7 |       -367.905 |    7.448 |        -0.127 |        -0.164 | [-0.357, 0.102] | [-0.396, 0.068] |

时间偏差变化范围约 0.091 s，平均 dx、平均 dy 的变化仍很小。窗口越大，目标函数越低，表明高频随机波动得到抑制；四组实验的 X、Y 置信区间都包含 0，因此平滑窗口改变不会改变“没有显著固定系统偏差”的判断。

### 4.2 分块大小和置信水平敏感性

以下表格列出 95% 置信水平下的区间。此组实验固定采用 5 点预平滑，时间偏差为约 `-367.906 s`。

| 分块大小（点） | 块数 | X 轴 95% 区间（米） | Y 轴 95% 区间（米） |
| -------------: | ---: | ------------------: | ------------------: |
|             10 |  300 |     [-0.354, 0.098] |     [-0.379, 0.052] |
|             20 |  150 |     [-0.369, 0.113] |     [-0.409, 0.082] |
|             50 |   60 |     [-0.384, 0.128] |     [-0.417, 0.090] |
|            100 |   30 |     [-0.385, 0.129] |     [-0.416, 0.089] |
...
因此可以在问题三中报告：在采用分块均值进行相关性稳健检验后，没有发现 X 轴或 Y 轴存在显著固定系统空间偏差。后续融合时可以保留随机误差建模，而不必对 X、Y 轴施加固定偏移修正。推荐正文使用 5 点预平滑、20 点分块、95% 置信水平及最终 11 点轨迹平滑作为默认设置。
